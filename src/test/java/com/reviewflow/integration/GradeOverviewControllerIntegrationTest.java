...
import com.reviewflow.grading.controller.GradeOverviewController;
...
class GradeOverviewControllerIntegrationTest {
  ...
  void getMyOverview_decodesCourseAndDelegatesToService() {
    ...
    GradeOverviewDto dto =
        GradeOverviewDto.builder()
            .courseCode("CS401")
  ...
  void getStudentOverview_decodesBothIdsAndDelegates() {
    ...
    GradeOverviewDto dto =
        GradeOverviewDto.builder()
            ...
            .groups(List.of())
            .build();
    when(gradeCalculationService.calculateStudentOverview(11L, 102L, 300L, UserRole.INSTRUCTOR))
        .thenReturn(dto);

    var response =
        controller()
            .getStudentOverview("COURSE_1", "STUDENT_2", principal(UserRole.INSTRUCTOR, 300L));
