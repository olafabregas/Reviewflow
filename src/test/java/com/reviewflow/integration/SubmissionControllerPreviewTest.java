...
import com.reviewflow.submission.controller.SubmissionController;
...
class SubmissionControllerPreviewTest {
  ...
  void preview_withValidSubmission_returns200WithPreviewDto() {
    ...
    when(submissionService.getPreviewUrl("sub123", 100L, UserRole.STUDENT)).thenReturn(previewDto);

    // Act
    ResponseEntity<ApiResponse<PreviewResponseDto>> response =
        controller.preview("sub123", mockUser);
  ...
  void preview_previewUrlReturnsExpiredTime() {
    ...
    when(submissionService.getPreviewUrl("sub456", 100L, UserRole.STUDENT)).thenReturn(previewDto);

    // Act
    ResponseEntity<ApiResponse<PreviewResponseDto>> response =
        controller.preview("sub456", mockUser);
  ...
  void preview_multipleContentTypes_returnsCorrectMimeType() {
    ...
    for (String mimeType : mimeTypes) {
      ...
      // Act
      ResponseEntity<ApiResponse<PreviewResponseDto>> response =
          controller.preview("subId", mockUser);
  ...
  void preview_instructorAccess_returns200() {
    ...
    // Act
    ResponseEntity<ApiResponse<PreviewResponseDto>> response =
        controller.preview("sub123", instructorUser);
