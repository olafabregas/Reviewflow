...
class GradeExportServiceTest {
  ...
  @InjectMocks private GradeExportService gradeExportService;
  ...
  void export_teamAssignment_successProducesCsvAndAudit() {
    ...
    when(teamMemberRepository.findByTeamIdsAndStatusWithUser(
            List.of(300L), TeamMemberStatus.ACCEPTED))
  ...
  void export_assignmentOutsideCourse_throwsAssignmentNotInCourse() {
    ...
    assertThrows(
        AssignmentNotInCourseException.class, () -> gradeExportService.export("C10", "A22", 1L));
  }
  ...
  void export_nonInstructorNonAdmin_throwsAccessDenied() {
    ...
    assertThrows(
        AccessDeniedException.class, () -> gradeExportService.export("C10", "A20", actorId));
    verify(auditService, never())
        .log(anyLong(), anyString(), anyString(), anyLong(), anyString(), any());
  }
  ...
  void export_unevaluatedSubmissionsAndNullRubric_producesBlankScoresWithZeroMaxScore() {
    ...
    User actor = User.builder().id(actorId).role(UserRole.ADMIN).build();
    User student =
        User.builder().id(201L).firstName("Bob").lastName("Jones").email("bob@example.com").build();
    Submission submission =
        Submission.builder()
            .id(702L)
            .assignment(assignment)
  ...
  void export_teamsWithNoAcceptedMembers_producesEmptyStudentsList() {
    ...
    when(evaluationRepository.findPublishedBySubmissionIds(List.of(501L)))
        .thenReturn(List.of(evaluation));
    when(teamMemberRepository.findByTeamIdsAndStatusWithUser(
            List.of(401L), TeamMemberStatus.ACCEPTED))
        .thenReturn(List.of());
  ...
  void export_filenameSanitizationEdgeCases_handlesSpecialCharsAndLength() {
    ...
    Assignment assignment =
        Assignment.builder()
            .id(assignmentId)
            .title("Project (Phase 1) â€” Deliverable #3!@#$%^&*()")
            .submissionType(SubmissionType.INDIVIDUAL)
  ...
  void export_csvInjectionAllCharacters_allArePrefixedWithTab() {
    ...
    User student3 =
        User.builder()
            ...
            .build();
    User student4 =
        User.builder().id(4L).firstName("White").lastName("@At").email("test@example.com").build();
  ...
  void export_mixedEvaluationStates_separatsDraftFromPublished() {
    ...
    User student1 =
        User.builder()
            ...
            .build();
    User student2 =
        User.builder().id(2L).firstName("Bob").lastName("Draft").email("bob@example.com").build();
    ...
    Evaluation publishedEval =
        Evaluation.builder()
            .id(1L)
            .submission(sub1)
            .isDraft(false)
            .totalScore(BigDecimal.valueOf(85))
            .publishedAt(Instant.parse("2026-03-23T10:00:00Z"))
            .build();
