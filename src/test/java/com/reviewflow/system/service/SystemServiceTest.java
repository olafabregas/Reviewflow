...
import com.reviewflow.shared.domain.*;
...
class SystemServiceTest {
  ...
  void testEvictCache_Success() {
    ...
    verify(auditService, times(1))
        .log(eq("CACHE_EVICTED"), eq("CACHE"), isNull(), any(Map.class), isNull());
  ...
  void testGetSafeConfig_Success() {
    ...
    config.forEach(
        (key, value) ->
            assertFalse(
                key.equalsIgnoreCase("jwt.secret")
  ...
  void testForceLogout_Success() {
    ...
    when(refreshTokenRepository.revokeAllForUser(targetUserId)).thenReturn(3);
  ...
  void testForceLogout_SelfLogout() {
    ...
    User selfUser = User.builder().id(actorId).email("self@test.com").build();
    ...
    // Act & Assert
    assertThrows(
        ForceLogoutBlockedException.class,
  ...
  void testUnlockTeam_Success() {
    ...
    Team team =
        Team.builder().id(teamId).name("Team A").isLocked(true).members(new ArrayList<>()).build();
    ...
    // Act
    UnlockTeamResponse result =
        systemService.unlockTeam(teamHashId, actorId, "Instructor on leave");
    ...
    verify(auditService, times(1))
        .log(eq("SYSTEM_TEAM_UNLOCKED"), eq("TEAM"), eq(teamId), any(Map.class), isNull());
    verify(eventPublisher, times(1)).publishEvent(any());
  }
  ...
  void testReopenEvaluation_Success() {
    ...
    ReopenEvaluationResponse result =
        systemService.reopenEvaluation(evalHashId, actorId, "Scoring correction");
    ...
    verify(auditService, atLeastOnce())
        .log(anyString(), anyString(), anyLong(), any(Map.class), isNull());
