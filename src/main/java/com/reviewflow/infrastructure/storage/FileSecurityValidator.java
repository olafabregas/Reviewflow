...
/**
 * Three-gate file security validation pipeline: Gate 1: Denylist + Allowlist Gate 2: MIME Type
 * Detection (Apache Tika) Gate 3: Structural Validation (format-specific)
 */
@Slf4j
@Service
...
public class FileSecurityValidator {
  ...
  public void validateWithConfig(MultipartFile file, ValidationConfig config) throws IOException {
    ...
    if (file.getSize() > config.maxFileSizeBytes()) {
      throw new AvatarTooLargeException(
          "Avatar exceeds maximum size of " + config.maxFileSizeBytes() + " bytes");
    }
  ...
  private void validateFromStream(
      InputStream inputStream, String filename, String extension, Long userId, String ipAddress)
      throws IOException {

    // ── GATE 2 — MIME Detection (Tika) ───────────────────────────────
    // Need to read stream twice, so save to temp file
    Path tempFile =
        Files.createTempFile("validation-", "-" + filename.replaceAll("[^a-zA-Z0-9.-]", "_"));
    try {
      ...
      if (isExecutableMime(detectedMime)) {
        ...
        log.warn(
            "EXECUTABLE_MIME_DETECTED user={} file={} mime={} ip={}",
            userId,
            filename,
            detectedMime,
            ipAddress);
        throw new InvalidMimeTypeException(
            "File content does not match its extension. Executable content is not permitted");
      }

      if (!isMimeAcceptable(detectedMime, extension)) {
        metrics.recordBlockedFileUpload("mime_mismatch");
        log.warn(
            "MIME_MISMATCH user={} file={} expected={} detected={} ip={}",
            userId,
            filename,
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  /**
   * Extracts just the filename from a full path. Handles both Windows (C:\path\file.txt) and Unix
   * (/path/file.txt) paths. This is necessary because Postman and some clients send full file
   * paths.
   */
    ...
    try {
      ...
      try (SevenZFile sevenZFile = new SevenZFile(tempFile.toFile())) {
        ...
        while (sevenZFile.getNextEntry() != null
            && entryCount < securityProperties.getMaxArchiveEntries()) {
          entryCount++;
        }
  ...
  private void validateRar(InputStream stream) throws Exception {
    try (ArchiveInputStream ais =
        new ArchiveStreamFactory().createArchiveInputStream("rar", stream)) {
      ...
      if (totalUncompressedSize > securityProperties.getMaxArchiveUncompressedSize()) {
        throw new ArchiveTooLargeException(
            "Archive uncompressed content exceeds the maximum allowed size of "
                + (securityProperties.getMaxArchiveUncompressedSize() / (1024 * 1024))
                + "MB");
      }
    }
