...
import com.reviewflow.shared.domain.AuditLog;
import com.reviewflow.shared.domain.User;
import com.reviewflow.admin.repository.AuditLogRepository;
...
public class AuditService {
  ...
  public void log(
    ...
    log(null, action, targetType, targetId, metadata, ip);
  }

  /** PRD-09: Overloaded log method that accepts actorId and Map-based metadata */
  @Transactional
  public void log(
      Long actorId,
      String action,
  ...
  public List<AuditLogDto> getSecurityEvents(int limit) {
    List<String> securityActions =
        Arrays.asList("USER_LOGIN_FAILED", "RATE_LIMIT_HIT", "FILE_BLOCKED");

    Page<AuditLog> events =
        auditLogRepository.findByActionInOrderByCreatedAtDesc(
            securityActions, PageRequest.of(0, Math.min(limit, 100)));
