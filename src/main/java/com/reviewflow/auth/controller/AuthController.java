...
public class AuthController {
  ...
  public ResponseEntity<ApiResponse<AuthUserResponse>> login(
    ...
    addCookie(
        response, ACCESS_COOKIE, result.accessToken(), authService.getAccessExpirationMs() / 1000);
    addCookie(
        response,
        REFRESH_COOKIE,
        result.refreshToken(),
        authService.getRefreshExpirationMs() / 1000);
  ...
  @Operation(
      ...
      description =
          "Extract access token from cookie for WebSocket authentication. WebSocket connections"
              + " cannot use cookies directly, so this endpoint provides token value for auth"
              + " header.")
  @ApiResponses({
    @io.swagger.v3.oas.annotations.responses.ApiResponse(
        responseCode = "200",
  ...
  public ResponseEntity<ApiResponse<Map<String, String>>> getTokenForWebSocket(
      HttpServletRequest request, @AuthenticationPrincipal ReviewFlowUserDetails user) {
