...
import com.reviewflow.assignment.exception.ModuleNotFoundException;
...
public class ModuleService {
  ...
  @Transactional
  public AssignmentModuleResponse create(
      Long courseId, Long actorId, String name, Integer displayOrder) {
  ...
  public CourseModulesResponse listByCourse(Long courseId) {
    ...
    List<AssignmentModule> modules =
        assignmentModuleRepository.findByCourseIdOrderByDisplayOrderAsc(courseId);
    List<AssignmentModuleResponse> moduleResponses =
        modules.stream()
    ...
    List<AssignmentModuleResponse.AssignmentSummary> unmoduled =
        assignmentRepository.findByCourseIdAndAssignmentModuleIsNull(courseId).stream()
            .sorted(
    ...
    return CourseModulesResponse.builder().modules(moduleResponses).unmoduled(unmoduled).build();
  }

  @Transactional
  public AssignmentModuleResponse update(
      Long moduleId, Long actorId, String name, Integer displayOrder) {
  ...
  @Transactional
  public AssignmentModuleMoveResponse assignToModule(
      Long assignmentId, Long moduleId, Long actorId) {
    ...
    auditService.log(
        actorId,
        "ASSIGNMENT_MOVED_TO_MODULE",
        "Assignment",
        assignmentId,
        "Moved assignment module",
        null);
  ...
  public CourseModulesResponse reorder(Long courseId, List<Long> orderedModuleIds, Long actorId) {
    ...
    auditService.log(
        actorId,
        "ASSIGNMENT_MODULES_REORDERED",
        "Course",
        courseId,
        "Reordered assignment modules",
        null);
    evictCourseModulesCache(courseId);
  ...
  public long delete(Long moduleId, Long actorId) {
    ...
    List<Assignment> linkedAssignments =
    ...
    auditService.log(
        actorId,
        "ASSIGNMENT_MODULE_DELETED",
        "AssignmentModule",
        moduleId,
        "Deleted module; affected assignments: " + affectedAssignments,
        null);
    evictCourseModulesCache(courseId);
  ...
  private AssignmentModuleResponse toResponse(
      AssignmentModule module, List<Assignment> assignments) {
    ...
    return AssignmentModuleResponse.AssignmentSummary.builder()
        ...
        .groupId(
            hashidService.encode(
                assignment.getAssignmentGroup() != null
                    ? assignment.getAssignmentGroup().getId()
                    : null))
        .groupName(
            assignment.getAssignmentGroup() != null
                ? assignment.getAssignmentGroup().getName()
                : null)
        .build();
  }
  ...
  private AssignmentModule getModule(Long moduleId) {
    return assignmentModuleRepository
        .findById(moduleId)
  ...
  private void ensureCourseWritable(Course course) {
    if (Boolean.TRUE.equals(course.getIsArchived())) {
      throw new CourseArchivedReadOnlyException(
          "Archived courses are read-only for module changes");
    }
  }
  ...
  private void validateReorderRequest(List<AssignmentModule> modules, List<Long> orderedModuleIds) {
    ...
    Set<Long> expectedIds =
        modules.stream().map(AssignmentModule::getId).collect(java.util.stream.Collectors.toSet());
    ...
    if (!expectedIds.equals(providedIds)) {
      throw new ValidationException(
          "Module order must include exactly all course modules", "VALIDATION_ERROR");
    }
  }
