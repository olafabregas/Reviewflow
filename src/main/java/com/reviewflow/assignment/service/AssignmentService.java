...
import com.reviewflow.assignment.exception.ModuleNotInCourseException;
...
import com.reviewflow.shared.domain.AssignmentModule;
...
import com.reviewflow.assignment.repository.AssignmentModuleRepository;
...
public class AssignmentService {
  ...
  public Assignment createAssignment(
    ...
    SubmissionType resolvedSubmissionType =
        submissionType != null ? submissionType : SubmissionType.INDIVIDUAL;
    ...
    if (resolvedSubmissionType == SubmissionType.TEAM) {
      ...
      if (maxScore == null || maxScore.compareTo(BigDecimal.ZERO) <= 0) {
        throw new ValidationException(
            "Max score must be greater than 0 for instructor-graded assignments",
            "VALIDATION_ERROR");
      }
      maxTeamSize = null;
      teamLockAt = null;
  ...
  public Assignment updateAssignment(
    ...
    if (submissionType != null && submissionType != a.getSubmissionType()) {
      boolean hasTeams = !teamRepository.findByAssignmentId(assignmentId).isEmpty();
      boolean hasSubmissions = !submissionRepository.findByAssignmentId(assignmentId).isEmpty();
    ...
    if (effectiveSubmissionType == SubmissionType.INDIVIDUAL) {
      ...
      if (maxScore != null) {
        ...
        if (instructorScoreRepository.existsByAssignmentIdAndIsPublishedTrue(assignmentId)
            && (a.getMaxScore() == null || a.getMaxScore().compareTo(maxScore) != 0)) {
          throw new ScoresExistException(
              "Cannot modify max score after published instructor scores exist");
        }
        a.setMaxScore(maxScore);
      } else if (a.getMaxScore() == null) {
        throw new ValidationException(
            "Max score is required for instructor-graded assignments", "VALIDATION_ERROR");
      }
  ...
  public Assignment updateAssignment(
      ...
      Long updaterId) {
    return updateAssignment(
        assignmentId, title, description, dueAt, maxTeamSize, null, teamLockAt, updaterId);
  }
    ...
    if (!newState) {
      ...
      if (!submissions.isEmpty()) {
        throw new BusinessRuleException(
            "Cannot unpublish assignment with existing submissions", "HAS_SUBMISSIONS");
      }
    }
  ...
  @CacheEvict(value = CacheNames.CACHE_ASSIGNMENT, key = "#assignmentId")
  @Transactional
  public RubricCriterion addRubricCriteria(
      Long assignmentId,
      String name,
      String description,
      int maxScore,
  ...
  @CacheEvict(value = CacheNames.CACHE_ASSIGNMENT, key = "#assignmentId")
  @Transactional
  public RubricCriterion updateRubricCriteria(
      Long assignmentId,
      Long criterionId,
      String name,
      String description,
  ...
  public void deleteRubricCriterion(Long assignmentId, Long criterionId, Long userId) {
    ...
    boolean hasScores = rubricScoreRepository.existsByCriterionId(criterionId);
    if (hasScores) {
      throw new BusinessRuleException(
          "Cannot delete criterion with existing evaluation scores", "HAS_SCORES");
    }
  ...
  public List<AssignmentResponse> listAssignmentsForUserWithDetails(
      Long userId, UserRole role, String status, Long courseId) {
  ...
  private AssignmentResponse toAssignmentResponseWithDetails(
      Assignment a, Long userId, boolean isStudent) {
    List<AssignmentResponse.RubricCriterionResponse> criteria =
        a.getRubricCriteria() != null
            ? a.getRubricCriteria().stream()
                .map(
                    c ->
                        AssignmentResponse.RubricCriterionResponse.builder()
    ...
    if (isStudent) {
      ...
      if (teamOpt.isPresent()) {
        ...
        List<Submission> submissions =
            submissionRepository.findByTeamIdOrderByVersionNumberDesc(team.getId());
        if (!submissions.isEmpty()) {
  ...
  public void deleteAssignment(Long assignmentId, Long userId) {
    ...
    if (Boolean.TRUE.equals(a.getIsPublished())) {
      throw new BusinessRuleException(
          "Unpublish the assignment before deleting", "ASSIGNMENT_PUBLISHED");
    }
    ...
    if (!submissions.isEmpty()) {
      throw new BusinessRuleException(
          "Cannot delete an assignment with existing submissions", "HAS_SUBMISSIONS");
    }
  ...
  public List<GradebookEntryResponse> getGradebookForAssignment(Long assignmentId, Long userId) {
    ...
    return teams.stream()
        .map(
            team -> {
              ...
              if (latestSubmission != null) {
                Optional<Evaluation> evalOpt =
                    evaluationRepository.findBySubmissionId(latestSubmission.getId());
              ...
              return GradebookEntryResponse.builder()
                  ...
                  .latestVersion(
                      latestSubmission != null ? latestSubmission.getVersionNumber() : null)
        ...
        .sorted(
            Comparator.comparing(
                GradebookEntryResponse::getTeamName,
                Comparator.nullsLast(Comparator.naturalOrder())))
        .toList();
  }

  public Assignment getAssignmentByIdWithAccessControl(
      Long assignmentId, Long userId, UserRole role) {
    Assignment a = getAssignmentById(assignmentId);
    ...
    if (role == UserRole.INSTRUCTOR) {
      if (!courseInstructorRepository.existsByCourseIdAndUserId(a.getCourse().getId(), userId)) {
    ...
    if (role == UserRole.STUDENT) {
      // Check enrollment
      if (!courseEnrollmentRepository.existsByCourseIdAndUserId(a.getCourse().getId(), userId)) {
        throw new ResourceNotFoundException("Assignment", assignmentId);
      }
  ...
  private AssignmentGroup resolveAssignmentGroup(
      Long courseId, Long groupId, boolean defaultToUncategorized) {
    if (groupId == null) {
      if (!defaultToUncategorized) {
        return null;
      }
      return assignmentGroupRepository
          .findByCourseIdAndIsUncategorizedTrue(courseId)
