package com.reviewflow.infra.monitoring;
...
/**
 * PRD-08: Centralised metric registration for ReviewFlow Replaces SecurityMetrics with
 * comprehensive coverage including: - Login security (success, failure, rate limiting) - Token
 * validation metrics - File upload security - S3 operations (upload/download timers) - Email
 * delivery (success/failure) - Notifications (per type) - Cache reload failures - Submission and
...
public class ReviewFlowMetrics {
  ...
  public ReviewFlowMetrics(MeterRegistry meterRegistry) {
    ...
    this.loginSuccessCounter =
        Counter.builder("reviewflow.security.login")
            .tag("result", "success")
    ...
    // ClamAV metrics
    this.clamavCleanCounter =
        Counter.builder("reviewflow.security.clamav_scan")
            .tag("result", "clean")
            .description("Clean files from ClamAV scan")
            .register(meterRegistry);

    this.clamavInfectedCounter =
        Counter.builder("reviewflow.security.clamav_scan")
            .tag("result", "infected")
            .description("Infected files detected by ClamAV")
            .register(meterRegistry);

    this.clamavErrorCounter =
        Counter.builder("reviewflow.security.clamav_scan")
            .tag("result", "error")
    ...
    // S3 operations
    this.s3UploadTimer =
        Timer.builder("reviewflow.s3.upload_duration")
            .description("S3 upload operation duration")
            .publishPercentiles(0.5, 0.95, 0.99)
            .register(meterRegistry);

    this.s3DownloadTimer =
        Timer.builder("reviewflow.s3.download_duration")
            .description("S3 download operation duration")
            .publishPercentiles(0.5, 0.95, 0.99)
            .register(meterRegistry);

    this.s3UploadFailureCounter =
        Counter.builder("reviewflow.s3.upload_failures")
            .description("S3 upload failures")
            .register(meterRegistry);

    this.s3DownloadFailureCounter =
        Counter.builder("reviewflow.s3.download_failures")
            .description("S3 download failures")
            .register(meterRegistry);

    // Email delivery
    this.emailSentCounter =
        Counter.builder("reviewflow.email.sent")
            .description("Emails successfully sent")
            .register(meterRegistry);

    this.emailFailedCounter =
        Counter.builder("reviewflow.email.failed")
            .description("Email send failures")
            .register(meterRegistry);

    // Notifications
    this.notificationSentCounter =
        Counter.builder("reviewflow.notifications.sent")
            .description("Notifications sent")
            .register(meterRegistry);

    // Cache operations
    this.cacheReloadFailureCounter =
        Counter.builder("reviewflow.cache.reload_failures")
            .description("Cache reload failures (refreshAfterWrite)")
            .register(meterRegistry);

    // Submission & evaluation
    this.submissionUploadedCounter =
        Counter.builder("reviewflow.submissions.uploaded")
            .description("Submissions uploaded")
            .register(meterRegistry);

    this.evaluationPublishedCounter =
        Counter.builder("reviewflow.evaluations.published")
            .description("Evaluations published")
            .register(meterRegistry);

    this.assignmentGroupCreatedCounter =
        Counter.builder("reviewflow.assignment_groups.created")
            .description("Assignment groups created")
            .register(meterRegistry);

    this.assignmentGroupUpdatedCounter =
        Counter.builder("reviewflow.assignment_groups.updated")
            .description("Assignment groups updated")
            .register(meterRegistry);

    this.assignmentGroupDeletedCounter =
        Counter.builder("reviewflow.assignment_groups.deleted")
            .description("Assignment groups deleted")
            .register(meterRegistry);

    this.assignmentGroupMovedCounter =
        Counter.builder("reviewflow.assignment_groups.moved")
