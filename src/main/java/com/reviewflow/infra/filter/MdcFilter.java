package com.reviewflow.infra.filter;
...
/**
 * PRD-08: MDC Filter for request correlation Injects traceId and requestId into every log line
 * automatically via Mapped Diagnostic Context Also populates endpoint, IP address, and user context
 * (populated later by JwtAuthenticationFilter)
 */
@Slf4j
@Component
...
public class MdcFilter extends OncePerRequestFilter {
  ...
  /**
   * Extract client IP address from request headers or socket Handles X-Forwarded-For (behind load
   * balancer), X-Real-IP, and direct connection
   */
