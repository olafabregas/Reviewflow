...
public class TeamService {
  ...
  public Team createTeam(Long assignmentId, String teamName, Long creatorId) {
    ...
    if (assignment.getTeamLockAt() != null && assignment.getTeamLockAt().isBefore(Instant.now())) {
      throw new DuplicateResourceException(
          "Team formation is closed for this assignment", "TEAM_FORMATION_CLOSED");
    }

    // Check if user is enrolled
    if (!courseEnrollmentRepository.existsByCourseIdAndUserId(
        assignment.getCourse().getId(), creatorId)) {
      throw new AccessDeniedException("You do not have access to this course");
    }

    // Check if user already has a team
    if (teamMemberRepository.existsByAssignmentIdAndUserIdAndStatus(
        assignmentId, creatorId, TeamMemberStatus.ACCEPTED)) {
      throw new BusinessRuleException(
          "You are already a member of a team for this assignment", "ALREADY_IN_TEAM");
    }

    // Check for duplicate team name
    if (teamRepository.existsByAssignmentIdAndName(assignmentId, teamName)) {
      throw new DuplicateResourceException(
          "Team name already exists for this assignment", "TEAM_NAME_EXISTS");
    }
  ...
  public List<Team> listTeamsForAssignment(Long assignmentId, Long userId, UserRole role) {
    if (role == UserRole.ADMIN || role == UserRole.INSTRUCTOR) {
      return teamRepository.findByAssignmentId(assignmentId);
    }
    return teamRepository
        .findByAssignmentIdAndMembersUserId(assignmentId, userId)
        .map(List::of)
        .orElse(List.of());
  }
  ...
  public Team renameTeam(Long teamId, String newName, Long userId) {
    ...
    // Check for duplicate team name
    if (teamRepository.existsByAssignmentIdAndNameAndIdNot(
        team.getAssignment().getId(), newName, teamId)) {
      throw new DuplicateResourceException(
          "Team name already exists for this assignment", "TEAM_NAME_EXISTS");
    }
  ...
  public TeamMember inviteMember(Long teamId, String inviteeEmail, Long inviterId) {
    ...
    if (Boolean.TRUE.equals(team.getIsLocked())
        || (assignment.getTeamLockAt() != null
            && assignment.getTeamLockAt().isBefore(Instant.now()))) {
      throw new AccessDeniedException("Team is locked");
    }
    ...
    long acceptedCount =
        team.getMembers().stream().filter(m -> m.getStatus() == TeamMemberStatus.ACCEPTED).count();
    if (acceptedCount >= assignment.getMaxTeamSize()) {
      throw new BusinessRuleException(
          "Team has reached maximum size of " + assignment.getMaxTeamSize(), "TEAM_FULL");
    }
            ...
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
                        "User with email " + inviteeEmail + " not found"));
    ...
    if (!courseEnrollmentRepository.existsByCourseIdAndUserId(
        assignment.getCourse().getId(), invitee.getId())) {
    ...
    if (teamMemberRepository.existsByAssignmentIdAndUserIdAndStatus(
        assignment.getId(), invitee.getId(), TeamMemberStatus.ACCEPTED)) {
      throw new BusinessRuleException(
          "Invitee already has a team for this assignment", "ALREADY_IN_TEAM");
    }

    // Check for duplicate pending invite
    if (teamMemberRepository.existsByTeamIdAndUserIdAndStatus(
        teamId, invitee.getId(), TeamMemberStatus.PENDING)) {
  ...
  public TeamMember respondToInvite(Long teamMemberId, boolean accept, Long userId) {
    ...
    if (accept) {
      ...
      if (teamMemberRepository.existsByAssignmentIdAndUserIdAndStatus(
          assignment.getId(), userId, TeamMemberStatus.ACCEPTED)) {
        throw new BusinessRuleException(
            "You are already a member of a team for this assignment", "ALREADY_IN_TEAM");
      }
      ...
      long acceptedCount =
          member.getTeam().getMembers().stream()
      ...
      List<TeamMember> otherPending =
          teamMemberRepository.findByAssignmentIdAndUserIdAndStatus(
              assignment.getId(), userId, TeamMemberStatus.PENDING);
      otherPending.stream()
  ...
  public void removeMember(Long teamId, Long memberUserId, Long requesterId) {
    ...
    boolean isCreator =
        team.getCreatedBy() != null && team.getCreatedBy().getId().equals(requesterId);
  ...
  public List<Team> autoAssignTeams(Long assignmentId, Long instructorId, int maxTeamSize) {
    ...
    if (!courseInstructorRepository.existsByCourseIdAndUserId(
        assignment.getCourse().getId(), instructorId)) {
    ...
    List<Long> enrolledUserIds =
        courseEnrollmentRepository.findByCourseId(assignment.getCourse().getId()).stream()
            .map(e -> e.getUser().getId())
            .toList();
    List<Long> unassigned =
        enrolledUserIds.stream()
            .filter(
                uid ->
                    !teamMemberRepository.existsByAssignmentIdAndUserIdAndStatus(
                        assignmentId, uid, TeamMemberStatus.ACCEPTED))
            .toList();
    ...
    int teamNumber = teamRepository.findByAssignmentId(assignmentId).size() + 1;
  ...
  public List<TeamMember> getPendingInvitesForUser(Long userId) {
    return teamMemberRepository.findByUserIdAndStatus(userId, TeamMemberStatus.PENDING);
  }
}
