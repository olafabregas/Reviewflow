...
import com.reviewflow.exception.*; // TODO [STYLE-AGENT]: fix structural violation
    // TODO [STYLE-AGENT]: fix structural violation
...
/**
 * AnnouncementService ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â manages lifecycle of announcements (create, publish, delete, read).
 * Enforces role-based permissions per PRD-04 roles matrix. Fires AnnouncementPublishedEvent on
 * publish for async notification + email dispatch.
 */
@Service
...
public class AnnouncementService {
  ...
  /**
   * Create a course announcement (draft). Only instructors of the course can create; students and
   * non-instructor admins cannot.
   *
   ...
   * @throws CourseNotOwnedException if actor is not an instructor of this course
   */
  @Transactional
  ...
  /**
   * Create a platform-wide announcement (draft). Only admins can create platform announcements.
   *
   * @param adminId actor user ID (must be ADMIN role)
   * @param title announcement title
   * @param body announcement body
   * @param recipientType recipient group (ALL_STUDENTS, ALL_INSTRUCTORS, ALL_USERS)
   * @return created Announcement entity (not yet published)
  ...
  /**
   * Publish a draft announcement. Only the creator or an admin can publish. Publishing is a one-way
   * transition. Fires AnnouncementPublishedEvent for async notification and email dispatch.
   *
   * @param announcementId announcement long ID
   * @param actorId actor user ID
   * @return published Announcement entity
   * @throws AnnouncementNotFoundException if announcement not found or not owned by actor
  ...
  public Announcement publish(Long announcementId, Long actorId) {
    ...
    AnnouncementPublishedEvent event =
        AnnouncementPublishedEvent.from(announcement, getDisplayName(actor));
  ...
  /**
   * Delete an announcement (hard delete, no soft delete). Only the creator or an admin can delete.
   *
   * @param announcementId announcement long ID
   * @param actorId actor user ID
  ...
  /**
   * Get published announcements for a course (paginated, by publish date DESC). Only
   * students/instructors enrolled in or teaching the course can view.
   *
   ...
   * @throws AccessDeniedException if actor is not enrolled/teaching the course
   */
