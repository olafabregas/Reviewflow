package com.reviewflow.extension.controller;

import com.reviewflow.extension.dto.request.CreateExtensionRequest;
import com.reviewflow.extension.dto.request.RespondExtensionRequest;
import com.reviewflow.shared.exception.ApiResponse;
import com.reviewflow.extension.dto.response.ExtensionRequestListItemResponse;
import com.reviewflow.extension.dto.response.ExtensionRequestResponse;
import com.reviewflow.shared.domain.ExtensionRequest;
import com.reviewflow.infrastructure.security.ReviewFlowUserDetails;
import com.reviewflow.extension.service.ExtensionRequestService;
...
public class ExtensionRequestController {
  ...
  @Operation(
      ...
      description =
          "Submit a deadline extension request for an assignment. Student-only. "
  ...
  public ResponseEntity<ApiResponse<ExtensionRequestResponse>> create(
    ...
    ExtensionRequest created =
        extensionRequestService.create(
            assignmentId, user.getUserId(), request.getReason(), request.getRequestedDueAt());
  ...
  @Operation(
      ...
      description =
          "Approve or reject a pending extension request. Instructor-only. Approved requests update"
              + " assignment due date and extend all team members' deadlines.")
  @ApiResponses({
    @io.swagger.v3.oas.annotations.responses.ApiResponse(
        responseCode = "200",
  ...
  public ResponseEntity<ApiResponse<ExtensionRequestResponse>> respond(
    ...
    ExtensionRequest updated =
        extensionRequestService.respond(
            extensionRequestId,
            user.getUserId(),
            request.getApprove(),
  ...
  @Operation(
      ...
      description =
          "Get paginated list of all extension requests for an assignment. "
  ...
  public ResponseEntity<ApiResponse<Page<ExtensionRequestListItemResponse>>> listByAssignment(
    ...
    Page<ExtensionRequest> page =
        extensionRequestService.getByAssignment(decodedAssignmentId, user.getUserId(), pageable);
  ...
  @Operation(
      ...
      description =
          "Get paginated list of authenticated student's extension requests. "
  ...
  public ResponseEntity<ApiResponse<Page<ExtensionRequestListItemResponse>>> listMine(
      Pageable pageable, @AuthenticationPrincipal ReviewFlowUserDetails user) {
    ...
    return ExtensionRequestResponse.builder()
        ...
        .studentId(
            request.getStudent() != null
                ? hashidService.encode(request.getStudent().getId())
                : null)
        .requestedById(
            request.getRequestedBy() != null
                ? hashidService.encode(request.getRequestedBy().getId())
                : null)
        .respondedById(
            request.getRespondedBy() != null
                ? hashidService.encode(request.getRespondedBy().getId())
                : null)
  ...
  private ExtensionRequestListItemResponse toListItemResponse(ExtensionRequest request) {
    return ExtensionRequestListItemResponse.builder()
        .id(hashidService.encode(request.getId()))
        .studentName(
            request.getStudent() != null ? request.getStudent().getFullNameOrEmail() : null)
