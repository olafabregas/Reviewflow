...
import com.reviewflow.team.dto.response.TeamInviteResponse;
...
public class StudentController {
  ...
  @Operation(
      ...
      description =
          "Get all pending team invitations for the authenticated student. "
  ...
  @Operation(
      ...
      description =
          "Get paginated list of all submissions uploaded by the authenticated student. "
  ...
  public ResponseEntity<ApiResponse<Page<SubmissionResponse>>> mySubmissions(
    ...
    Page<SubmissionResponse> page =
        submissionService
            .getMySubmissions(user.getUserId(), pageable)
  ...
  @Operation(
      ...
      description =
          "Get paginated list of all evaluations for the authenticated student's submissions. "
  ...
  public ResponseEntity<ApiResponse<Page<EvaluationResponse>>> myEvaluations(
    ...
    Page<EvaluationResponse> page =
        evaluationService.getMyEvaluations(user.getUserId(), pageable).map(this::toEvalResponse);
    return ResponseEntity.ok(ApiResponse.ok(page));
  }
  ...
  private TeamInviteResponse toInviteResponse(TeamMember m) {
    return TeamInviteResponse.builder()
        ...
        .assignmentId(
            m.getAssignment() != null ? hashidService.encode(m.getAssignment().getId()) : null)
        .assignmentTitle(m.getAssignment() != null ? m.getAssignment().getTitle() : null)
        .courseCode(
            m.getAssignment() != null && m.getAssignment().getCourse() != null
                ? m.getAssignment().getCourse().getCode()
                : null)
        .invitedByName(
            m.getInvitedBy() != null
                ? m.getInvitedBy().getFirstName() + " " + m.getInvitedBy().getLastName()
                : null)
