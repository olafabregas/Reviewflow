...
import com.reviewflow.grading.exception.ScoreExceedsMaxException;
...
public class InstructorScoreService {
  ...
  public InstructorScoreResponse create(
    ...
    auditService.log(
        actorId,
        "INSTRUCTOR_SCORE_SAVED",
        "InstructorScore",
        saved.getId(),
        "Saved instructor score draft",
        null);
  ...
  @Transactional
  public InstructorScoreResponse update(
      Long scoreId, Long actorId, BigDecimal score, String comment) {
    ...
    auditService.log(
        actorId,
        "INSTRUCTOR_SCORE_UPDATED",
        "InstructorScore",
        saved.getId(),
        "Updated instructor score draft",
        null);
  ...
  public InstructorScoreListResponse listByAssignment(
      Long assignmentId, Long actorId, UserRole actorRole, int page, int size) {
    Assignment assignment = getAssignment(assignmentId);
    if (actorRole == UserRole.ADMIN || actorRole == UserRole.SYSTEM_ADMIN) {
      // allowed for list/view
    } else if (!courseInstructorRepository.existsByCourseIdAndUserId(
    ...
    Page<InstructorScore> scorePage =
        instructorScoreRepository.findByAssignmentId(assignmentId, PageRequest.of(page, size));
    List<InstructorScoreListResponse.InstructorScoreItem> items =
        scorePage.getContent().stream()
            .map(
                score ->
                    InstructorScoreListResponse.InstructorScoreItem.builder()
                        ...
                        .studentName(
                            score.getStudent() != null
                                ? score.getStudent().getFullNameOrEmail()
                                : null)
                        .teamId(
                            hashidService.encode(
                                score.getTeam() != null ? score.getTeam().getId() : null))
    ...
    long total = instructorScoreRepository.countByAssignmentId(assignmentId);
    long published = instructorScoreRepository.countByAssignmentIdAndIsPublishedTrue(assignmentId);
    long draft = instructorScoreRepository.countByAssignmentIdAndIsPublishedFalse(assignmentId);
    long notEntered = Math.max(0, expectedTargetCount(assignment) - total);

    return InstructorScoreListResponse.builder()
        .scores(items)
        .summary(
            InstructorScoreListResponse.Summary.builder()
                .total(total)
                .published(published)
                .draft(draft)
  ...
  public int publishAll(Long assignmentId, Long actorId) {
    Assignment assignment = getAssignment(assignmentId);
    ensureCanManage(assignment.getCourse().getId(), actorId);
    List<InstructorScore> drafts =
        instructorScoreRepository.findByAssignmentIdAndIsPublishedFalse(assignmentId);
    drafts.forEach(
        score -> {
          score.setIsPublished(true);
          score.setPublishedAt(Instant.now());
    ...
    auditService.log(
        actorId,
        "INSTRUCTOR_SCORES_BULK_PUBLISHED",
        "Assignment",
        assignmentId,
  ...
  public InstructorScoreResponse reopen(Long scoreId, Long actorId, String reason) {
    ...
    InstructorScore saved = instructorScoreRepository.save(score);
    auditService.log(
        actorId, "INSTRUCTOR_SCORE_REOPENED", "InstructorScore", saved.getId(), reason, null);
    gradeCalculationService.evictCourseGradeCaches(score.getAssignment().getCourse().getId());
    return toResponse(saved);
  }
  ...
  private InstructorScore getScore(Long scoreId) {
    return instructorScoreRepository
        .findById(scoreId)
  ...
  private long expectedTargetCount(Assignment assignment) {
    if (assignment.getSubmissionType() == SubmissionType.TEAM) {
      return teamRepository.findByAssignmentId(assignment.getId()).size();
    }
    return courseEnrollmentRepository.countByCourseId(assignment.getCourse().getId());
  }

  private InstructorScoreResponse toResponse(InstructorScore score) {
    BigDecimal percent =
        score.getMaxScore() == null || score.getMaxScore().compareTo(BigDecimal.ZERO) == 0
            ? BigDecimal.ZERO
            : score
                .getScore()
    ...
    return InstructorScoreResponse.builder()
        ...
        .studentId(
            hashidService.encode(score.getStudent() != null ? score.getStudent().getId() : null))
