...
import com.reviewflow.grading.exception.CannotCommitWithErrorsException;
import com.reviewflow.grading.exception.ImportSessionExpiredException;
...
public class CsvImportService {
  ...
  public InstructorScoreImportPreviewResponse dryRun(
      Long assignmentId, Long actorId, MultipartFile file) {
    ...
    for (int i = 1; i < rows.size(); i++) {
      ...
      try {
        if (teamMode) {
          ...
          validRows.add(
              InstructorScoreImportPreviewResponse.ValidRow.builder()
                  .teamId(teamRawId)
                  .score(score)
                  .comment(comment)
                  .build());
    ...
    if (!validRows.isEmpty()) {
      ...
      if (cache != null) {
        cache.put(
            importId,
            new CachedImportSession(
                assignmentId, actorId, teamMode, validRows, errors, warnings, Instant.now()));
      }
    }
  ...
  @Transactional
  public InstructorScoreImportCommitResponse commit(
      Long assignmentId, Long actorId, String importId) {
    Cache cache = cacheManager.getCache(CacheNames.CACHE_CSV_IMPORTS);
    if (cache == null) {
    ...
    if (session == null
        || !session.assignmentId().equals(assignmentId)
    ...
    for (InstructorScoreImportPreviewResponse.ValidRow row : session.validRows()) {
      if (session.teamMode()) {
        ...
        instructorScoreService.create(
            assignmentId, actorId, null, teamId, row.getScore(), row.getComment());
        if (existed) {
          updated++;
        } else {
          created++;
        ...
        User student =
            userRepository
                .findByEmail(row.getStudentEmail())
        ...
        boolean existed =
            instructorScoreRepository
                ...
                .isPresent();
        instructorScoreService.create(
            assignmentId, actorId, student.getId(), null, row.getScore(), row.getComment());
        if (existed) {
          updated++;
        } else {
          created++;
    ...
    return InstructorScoreImportCommitResponse.builder()
        ...
        .updated(updated)
        .message(
            created + updated + " scores saved as drafts. Use publish-all to release to students.")
        .build();
  }
  ...
  private InstructorScoreImportPreviewResponse.RowIssue issue(
      int row, String email, String teamId, String error, String warning) {
  ...
  private record CachedImportSession(
      Long assignmentId,
      Long actorId,
      boolean teamMode,
