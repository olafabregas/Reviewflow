...
public class GradeExportService {
  ...
  public ExportResult export(String courseHashId, String assignmentHashId, Long actorUserId) {
    ...
    boolean isCourseInstructor =
        courseInstructorRepository.existsByCourseIdAndUserId(courseId, actorUserId);
    if (!isAdmin && !isCourseInstructor) {
      throw new AccessDeniedException(
          "You do not have permission to export grades for this assignment");
    }
    ...
    String[] headers =
        assignment.getSubmissionType() == SubmissionType.TEAM ? TEAM_HEADERS : INDIVIDUAL_HEADERS;
  ...
  private List<String[]> buildTeamRows(Assignment assignment, int maxScore) {
    ...
    Map<Long, List<TeamMember>> acceptedMembersByTeamId =
  ...
  private Map<Long, Evaluation> findPublishedEvaluationsBySubmissionId(
      List<Submission> submissions) {
  ...
  private String buildFileName(String courseCode, String assignmentTitle) {
    String safeCourseCode =
        (courseCode == null ? "course" : courseCode.trim()).replaceAll("[^A-Za-z0-9_-]", "");
    String normalized =
        assignmentTitle == null ? "assignment" : assignmentTitle.toLowerCase(Locale.ROOT);
  ...
  private String[] csvSafeRow(String... values) {
    return Arrays.stream(values).map(this::csvSafeCell).toArray(String[]::new);
  }
