...
import com.reviewflow.course.exception.NotEnrolledException;
import com.reviewflow.shared.exception.ResourceNotFoundException;
import com.reviewflow.grading.dto.response.AssignmentGradeDto;
import com.reviewflow.grading.dto.response.ClassRosterDto;
import com.reviewflow.grading.dto.response.ClassStatsDto;
import com.reviewflow.grading.dto.response.GradeOverviewDto;
import com.reviewflow.grading.dto.response.GroupGradeDto;
...
public class GradeCalculationService {
  ...
  @Transactional(readOnly = true)
  public GradeOverviewDto calculateStudentOverview(
      Long courseId, Long studentId, Long actorId, UserRole actorRole) {
    ensureCourseExists(courseId);
  ...
  @Transactional(readOnly = true)
  public ClassRosterDto calculateRoster(
      Long courseId,
      Long actorId,
      UserRole actorRole,
      String sortBy,
    ...
    if (actorRole == UserRole.ADMIN) {
      auditService.log(
          actorId,
          "GRADE_OVERVIEW_ROSTER_VIEWED",
          "Course",
          courseId,
          "ADMIN viewed class roster",
          null);
    }
    ...
    if (atRiskOnly) {
      students = students.stream().filter(s -> Boolean.TRUE.equals(s.getAtRisk())).toList();
    }
  ...
  public GradeOverviewDto calculateOverviewCached(Long courseId, Long studentId) {
    try {
      ...
      List<Assignment> allAssignments =
          groups.stream().flatMap(group -> group.getAssignments().stream()).toList();
      List<Long> assignmentIds = allAssignments.stream().map(Assignment::getId).toList();

      Map<Long, Team> teamByAssignmentId =
          teamRepository.findByAssignmentIdsAndMemberUserId(assignmentIds, studentId).stream()
              .collect(
                  Collectors.toMap(
                      team -> team.getAssignment().getId(), team -> team, (a, b) -> a));
      ...
      Map<Long, Submission> latestIndividualByAssignmentId =
          submissionRepository
              ...
              .collect(
                  Collectors.toMap(
                      submission -> submission.getAssignment().getId(), submission -> submission));

      Map<Long, Submission> latestTeamByAssignmentId = new HashMap<>();
      for (Team team : new HashSet<>(teamByAssignmentId.values())) {
        List<Submission> latestByTeam =
            submissionRepository.findLatestByAssignmentIdsAndTeamId(assignmentIds, team.getId());
        latestByTeam.forEach(
            submission ->
                latestTeamByAssignmentId.put(submission.getAssignment().getId(), submission));
      }

      Set<Long> submissionIds = new HashSet<>();
      latestIndividualByAssignmentId
          .values()
          .forEach(submission -> submissionIds.add(submission.getId()));
      latestTeamByAssignmentId
          .values()
          .forEach(submission -> submissionIds.add(submission.getId()));
      ...
      Map<Long, Evaluation> publishedEvaluationBySubmissionId =
          submissionIds.isEmpty()
              ...
              : evaluationRepository
                  .findPublishedFinalBySubmissionIds(new ArrayList<>(submissionIds))
                  .stream()
                  .collect(
                      Collectors.toMap(
                          evaluation -> evaluation.getSubmission().getId(),
                          evaluation -> evaluation));

      Map<Long, InstructorScore> instructorScoreByAssignmentId =
          instructorScoreRepository
              .findByAssignmentIdInAndStudentIdAndIsPublishedTrue(assignmentIds, studentId)
              .stream()
      ...
      BigDecimal configuredWeight =
          groups.stream()
              .map(
                  assignmentGroup ->
                      assignmentGroup.getWeight() == null
                          ? BigDecimal.ZERO
                          : assignmentGroup.getWeight())
      ...
      String weightWarning =
          configuredWeight.compareTo(BigDecimal.valueOf(100)) == 0
              ? null
              : "Grade weights total "
                  + configuredWeight.stripTrailingZeros().toPlainString()
      ...
      log.error(
          "Unexpected error calculating grade overview for courseId={}, studentId={}",
          courseId,
          studentId,
          ex);
  ...
  private GroupGradeDto calculateGroup(
        ...
        group.getAssignments().stream()
            .sorted(
                Comparator.comparing(
                    Assignment::getDueAt, Comparator.nullsLast(Comparator.naturalOrder())))
            .toList();

    for (Assignment assignment : sortedAssignments) {
      assignmentGrades.add(
          buildAssignmentGrade(
              assignment,
    ...
    List<AssignmentGradeDto> published =
        assignmentGrades.stream()
            .filter(
                row ->
                    row.getScore() != null
                        && row.getMaxScore() != null
                        && row.getMaxScore().compareTo(BigDecimal.ZERO) > 0)
            .collect(Collectors.toCollection(ArrayList::new));

    int dropLowestN = group.getDropLowestN() == null ? 0 : group.getDropLowestN();
    if (dropLowestN > 0 && dropLowestN < published.size()) {
      List<AssignmentGradeDto> sortedForDrop =
          published.stream()
              .sorted(
                  Comparator.comparing(
                      AssignmentGradeDto::getPercent, Comparator.nullsLast(BigDecimal::compareTo)))
              .toList();

      Set<String> droppedIds =
          sortedForDrop.stream()
              .limit(dropLowestN)
    ...
    List<AssignmentGradeDto> effective =
        assignmentGrades.stream()
            .filter(
                row ->
                    row.getScore() != null
    ...
    if (published.isEmpty()) {
      ...
      BigDecimal totalScore =
          effective.stream()
              .map(AssignmentGradeDto::getScore)
              .reduce(BigDecimal.ZERO, BigDecimal::add);
      BigDecimal totalMax =
          effective.stream()
              .map(AssignmentGradeDto::getMaxScore)
              .reduce(BigDecimal.ZERO, BigDecimal::add);
      groupScorePercent =
  ...
  private AssignmentGradeDto buildAssignmentGrade(
    ...
    if (assignment.getSubmissionType() == SubmissionType.INSTRUCTOR_GRADED) {
      ...
      Submission latestSubmission =
          assignment.getSubmissionType() == SubmissionType.TEAM
    ...
    return AssignmentGradeDto.builder()
        ...
        .moduleId(
            hashidService.encode(
                assignment.getAssignmentModule() != null
                    ? assignment.getAssignmentModule().getId()
                    : null))
        .moduleName(
            assignment.getAssignmentModule() != null
                ? assignment.getAssignmentModule().getName()
                : null)
        .score(score)
        .maxScore(maxScore)
        .percent(percent)
  ...
  private BigDecimal resolveMaxScore(Assignment assignment) {
    ...
    int rubricMax =
        assignment.getRubricCriteria() == null
            ? 0
            : assignment.getRubricCriteria().stream()
                .mapToInt(
                    criterion -> criterion.getMaxScore() == null ? 0 : criterion.getMaxScore())
                .sum();
  ...
  private ClassRosterDto buildRoster(Long courseId) {
    ...
    for (CourseEnrollment enrollment : enrollments) {
      ...
      students.add(
          ClassRosterDto.StudentStandingDto.builder()
    ...
    BigDecimal average =
        sortedStandings.isEmpty()
            ? null
            : sortedStandings.stream()
                .reduce(BigDecimal.ZERO, BigDecimal::add)
                .divide(BigDecimal.valueOf(sortedStandings.size()), 2, RoundingMode.HALF_UP);
    BigDecimal highest =
        sortedStandings.isEmpty() ? null : sortedStandings.get(sortedStandings.size() - 1);
    ...
    int atRiskCount =
        (int) students.stream().filter(s -> Boolean.TRUE.equals(s.getAtRisk())).count();
  ...
  private Comparator<ClassRosterDto.StudentStandingDto> buildComparator(String sortBy) {
    ...
    return switch (value) {
      case "name" ->
          Comparator.comparing(
              ClassRosterDto.StudentStandingDto::getName,
              Comparator.nullsLast(String::compareToIgnoreCase));
      case "email" ->
          Comparator.comparing(
              ClassRosterDto.StudentStandingDto::getEmail,
              Comparator.nullsLast(String::compareToIgnoreCase));
      default ->
          Comparator.comparing(
  ...
  private void verifyRosterAccess(Long courseId, Long actorId, UserRole role) {
    ...
    if (role == UserRole.INSTRUCTOR
        && courseInstructorRepository.existsByCourseIdAndUserId(courseId, actorId)) {
      return;
    }
