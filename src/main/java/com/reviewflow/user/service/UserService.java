...
import com.reviewflow.service.EXIFStripperService;
import com.reviewflow.service.FileSecurityValidator;
import com.reviewflow.service.PasswordPolicyService;
import com.reviewflow.service.ValidationConfig;
...
public class UserService {
  ...
  public Page<User> listUsersFiltered(
      UserRole role, Boolean isActive, String search, Pageable pageable) {
    // Validate search length
    if (search != null && search.trim().length() > 0 && search.trim().length() < 2) {
      throw new ValidationException(
          "Search term must be at least 2 characters", "VALIDATION_ERROR");
    }
  ...
  public UserDetailResponse getUserByIdWithCounts(Long id) {
    ...
    long enrollmentCount = courseEnrollmentRepository.countByUserId(id);
    long instructorCount = courseInstructorRepository.countByUserId(id);
    long courseCount = enrollmentCount + instructorCount;

    // Calculate teamCount
    long teamCount = teamMemberRepository.countByUserId(id);
  ...
  public AuthUserResponse uploadAvatar(Long userId, MultipartFile file, String ipAddress) {
    ...
    try {
      storageService.store(
          key, new ByteArrayInputStream(strippedBytes), strippedBytes.length, mimeType);
  ...
  public User createUser(
    ...
    eventPublisher.publishEvent(
        new WelcomeEmailEvent(user.getId(), user.getEmail(), user.getFirstName()));
    return user;
  }
  ...
  public void deactivateUser(Long id, Long currentUserId) {
    ...
    if (id.equals(currentUserId)) {
      throw new BusinessRuleException(
          "Cannot deactivate your own account", "CANNOT_DEACTIVATE_SELF");
    }
    ...
    // Revoke all active refresh tokens and increment token version for immediate access token
    // invalidation
  ...
  public void reactivateUser(Long id) {
    ...
    adminStatsService.evictStats();
    eventPublisher.publishEvent(
        new AccountReactivatedEmailEvent(user.getEmail(), user.getFirstName()));
  }
