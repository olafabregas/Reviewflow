...
/**
 * AnnouncementController — handles all announcement endpoints per PRD-04. Thin translators: all
 * business logic delegated to AnnouncementService.
 */
@RestController
...
public class AnnouncementController {
  ...
  /**
   * POST /courses/{id}/announcements Create a course announcement (draft). Only instructors of the
   * course can create.
   *
   * @param courseId encoded course hashid
   * @param request title and body (recipientType ignored for course announcements)
  ...
  @Operation(
      ...
      description =
          "Create a draft announcement for a course. Only instructors of the course can create. "
  ...
  public ResponseEntity<ApiResponse<AnnouncementResponse>> createCourseAnnouncement(
    ...
    Announcement announcement =
        announcementService.createCourseAnnouncement(
            courseId, user.getUserId(), request.getTitle(), request.getBody());
        ...
        AnnouncementResponse.builder()
            ...
            .courseId(
                announcement.getCourse() != null
                    ? hashidService.encode(announcement.getCourse().getId())
                    : null)
  ...
  /**
   * POST /admin/announcements Create a platform-wide announcement (draft). Only admins can create.
   *
   * @param request title, body, recipientType (required for platform announcements)
  ...
  @Operation(
      ...
      description =
          "Create a platform-wide announcement visible across all courses. Admin-only. "
              + "Announcement starts in draft state and must be published to be visible.")
  @ApiResponses({
    @io.swagger.v3.oas.annotations.responses.ApiResponse(
        responseCode = "201",
  ...
  public ResponseEntity<ApiResponse<AnnouncementResponse>> createPlatformAnnouncement(
    ...
    Announcement announcement =
        announcementService.createPlatformAnnouncement(
            user.getUserId(), request.getTitle(), request.getBody(), request.getRecipientType());
  ...
  /**
   * PATCH /announcements/{id}/publish Publish a draft announcement. Only the creator or an admin
   * can publish.
   *
   * @param id encoded announcement hashid
   * @param user authenticated user (must be creator or admin)
   * @return 200 OK with updated AnnouncementResponse (isDraft=false, publishedAt set)
   */
  @Operation(
      summary = "Publish announcement",
      description =
          "Publish a draft announcement making it visible to users. Only creator or admin can"
              + " publish. Sets publishedAt timestamp and sends notifications.")
  @ApiResponses({
    @io.swagger.v3.oas.annotations.responses.ApiResponse(
        responseCode = "200",
  ...
  /**
   * DELETE /announcements/{id} Delete an announcement (hard delete). Only the creator or an admin
   * can delete.
   *
   * @param id encoded announcement hashid
   * @param user authenticated user (must be creator or admin)
  ...
  @Operation(
      ...
      description =
          "Delete an announcement permanently. Only creator or admin can delete. "
  ...
  /**
   * GET /courses/{id}/announcements?page=0&size=20 List published announcements for a course
   * (paginated, by publishedAt DESC). Only enrolled students or course instructors can view.
   *
   * @param courseId encoded course hashid
  ...
  @Operation(
      ...
      description =
          "Get paginated list of published announcements for a course. Only enrolled students and"
              + " instructors can view. Sorted by publishedAt descending.")
  @ApiResponses({
    @io.swagger.v3.oas.annotations.responses.ApiResponse(
        responseCode = "200",
  ...
  public ResponseEntity<ApiResponse<PaginatedAnnouncementResponse>> listCourseAnnouncements(
    ...
    Page<Announcement> page =
        announcementService.getByCourse(courseIdLong, user.getUserId(), pageable);

    PaginatedAnnouncementResponse response =
        PaginatedAnnouncementResponse.builder()
            .content(
                page.getContent().stream()
                    .map(
                        a ->
                            PaginatedAnnouncementResponse.AnnouncementListItem.builder()
